...
# Why xfail: inherited from the dapper-era port. Pinned orbax-checkpoint
# silently accepts a config whose abstract shape disagrees with the saved
# arrays (no exception, restored arrays take on the abstract shape). Left
# in place as a regression beacon for whenever orbax tightens this.
@pytest.mark.xfail(
    reason="orbax does not currently raise on shape mismatch", strict=True
)
def test_from_checkpoint_raises_on_shape_mismatch() -> None:
    ...
    with pytest.raises(Exception) as exc_info:
        flax_nnx_checkpoint_util.from_checkpoint(drifted, _SimpleMlp)
...
def test_blob_accepts_legacy_dapper_shape() -> None:
    payload = b"legacy dapper bytes"
    legacy_dict = {
        "dapper_is_blob": True,
    ...
    loaded = types.Blob.model_validate(legacy_dict)
    assert loaded.as_bytes() == payload
    assert loaded.file_suffix == ".tar.gz"

    via_json = types.Blob.model_validate_json(json.dumps(legacy_dict))
    assert via_json.as_bytes() == payload


def test_checkpoint_loads_from_legacy_dapper_json(tmp_path: Path) -> None:
    """JSON dumped by `dapper.read(...).model_dump_json()` deserializes
    straight into the new pydantic checkpoint via `model_validate_json`."""
    model = _make_simple_mlp(hidden=8)
    new_ckpt = flax_nnx_checkpoint_util.to_checkpoint(
        model, _SimpleMlpConfig(hidden=8), _SimpleMlpCheckpoint
    )
    archive_bytes = new_ckpt.archive.as_bytes()
    legacy_payload = {
        "archive": {
            "dapper_is_blob": True,
        ...
        "config": new_ckpt.config.model_dump(),
    }
    legacy_path = tmp_path / "legacy_dump.json"
    legacy_path.write_text(json.dumps(legacy_payload))

    loaded = _SimpleMlpCheckpoint.model_validate_json(legacy_path.read_text())
    assert loaded.archive.as_bytes() == archive_bytes
    assert loaded.config == new_ckpt.config

    restored = flax_nnx_checkpoint_util.from_checkpoint(loaded, _SimpleMlp)
    x = jnp.ones((1, 4))
    np.testing.assert_allclose(np.asarray(model(x)), np.asarray(restored(x)), atol=0)
