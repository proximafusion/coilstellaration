"""Generic categorical + numeric sampling over pydantic BaseModel templates.

Samples one pydantic BaseModel instance from another whose fields carry either
``tuple[T, T]`` numeric ranges or ``frozenset[T]`` categorical choice
sets. Scalar and ``None`` fields are copied through unchanged.
"""
...
from constellaration_update.utils.types import ScalarInt
...
def _classify(bounds: pydantic.BaseModel) -> list[tuple[str, Kinds, object]]:
