"""Save/load helpers for Flax NNX modules via orbax-checkpoint + pydantic."""
...
_CkptT = TypeVar("_CkptT", bound=flax_nnx_checkpoint_types.FlaxNnxCheckpoint)
...
def to_checkpoint(
    ...
    checkpoint_cls: type[_CkptT] = flax_nnx_checkpoint_types.FlaxNnxCheckpoint,  # type: ignore[assignment]
) -> _CkptT:
...
def from_checkpoint(
    checkpoint: flax_nnx_checkpoint_types.FlaxNnxCheckpoint,
    module_cls: type[_M],
) -> _M:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        _untar_gz_into(checkpoint.archive, tmp_path)
    ...
    return cast(_M, nnx.merge(graphdef, restored_state))
