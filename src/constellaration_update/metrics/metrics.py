...
from constellaration_update.utils.types import ScalarFloat, runtime_check_array_sizes
...
def metrics_to_targets(
    metrics: constellaration_update_types.ConStellarationUpdateMetrics,
) -> constellaration_update_types.MetricsTargets:
    """Reduce `ConStellarationUpdateMetrics` to the 6 conditioning scalars."""
    return constellaration_update_types.MetricsTargets(
        min_normalized_coil_to_coil_distance=float(
            jnp.min(jnp.asarray(metrics.normalized_coil_to_coil_min_distances))
        ),
        min_normalized_coil_to_plasma_distance=float(
            jnp.min(jnp.asarray(metrics.normalized_coil_to_plasma_min_distances))
        ),
        max_normalized_coil_curvature=float(
            jnp.max(jnp.asarray(metrics.normalized_coil_curvatures))
        ),
        max_normalized_field_error=float(
            jnp.max(jnp.asarray(metrics.normalized_field_error))
        ),
        on_axis_average_magnetic_field=float(metrics.on_axis_average_magnetic_field),
        toroidal_flux=float(metrics.toroidal_flux),
    )


def metrics_to_requirement_metrics(
    metrics: constellaration_update_types.ConStellarationUpdateMetrics,
) -> constellaration_update_types.RequirementMetrics:
    """Reduce `ConStellarationUpdateMetrics` to the 4 requirement-aligned scalars."""
